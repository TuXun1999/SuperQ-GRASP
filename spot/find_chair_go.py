...
def estimate_obj_pose_hand(bbox, image_response, distance):
    ...
    result = SE3Pose(x=root[2], y=-root[1], z=-root[0], rot=Quat(w=1, x=0, y=0, z=0))
    result.position.x = root[2]
    result.position.y = -root[1]
    result.position.z = -root[0]
    print(result)
    return result
...
def find_chair_go(options):
    ...
    with bosdyn.client.lease.LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        drop_position_rt_vision, heading_rt_vision = compute_stand_location_and_yaw(
                vision_tform_obj, robot_state_client, distance_margin=0.8)
